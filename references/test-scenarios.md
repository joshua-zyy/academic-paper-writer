...
1. 是否在正确位置执行 `Step 6.3`
2. 若适用，是否执行 `Step 6.5`
3. 是否输出 `prose_debt`、`failed_items`、`thin_draft`
4. `Verification Status` 是否与预期一致
5. `Paper Body` 是否满足正面约束，而不是只形式合规
...
- 重点检查：
  ...
  - 若正文仍像提纲，`Step 6.3` 是否判 `prose_debt: open`
- 期望：
  - `Step 6.3`: 对早期章节通常应先判 `failed`
  - `Step 7`: 视内容密度决定 `thin_draft`
  - `Step 8`: 若文献缺口未闭合，不能轻易判 `passed`
...
- 期望：
  - 若只有背景堆叠和贡献列表，`Step 6.3` 判 `prose_debt: open`
  - `failed_items` 包含缺少论证链
  - 完成 `Prose Rewrite` 后再进入 `Step 7`
...
- 期望：
  - 罗列式草稿应在 `Step 6.3` 被拦下
  - 若结构上仍过薄，`Step 7` 还应给出 `thin_draft: yes`
...
- 重点检查：
  - `Step 6.3` 是否先检查通用 prose 问题
  - `Step 6.5` 是否额外检查模块级叙事与推断降级口吻
  - 无法稳妥恢复的动机是否进入 `[RATIONALE_NEEDED: ...]` 或 sidecar
- 期望：
  - 若存在公式罗列或“从实现看”口吻，`Step 6.5` 不能放行
  - 若两轮 `Step 6.3 + Prose Rewrite` 后仍有通用 prose 问题，最终不得 `passed`
...
- 重点检查：
  ...
  - 是否把内部验证包装成强泛化结论
- 期望：
  - `Step 6.3` 关注 prose 形式，`Step 7` 关注结果节是否缺协议或风险说明
  - 若核心结果可写但关键协议缺口需要外部证据，`Step 8` 可判 `blocked`
...
- 重点检查：
  - 第一轮 `Prose Rewrite` 后是否重新执行 `Step 6.3`
  - 第二轮后是否允许进入 `Step 7`
- 期望：
  - `Step 6.3 + Prose Rewrite` 循环最多 `2` 轮
  - 第二轮后即使允许继续，若 `prose_debt` 仍 open，`Step 8` 也必须判 `failed`
