...
`Step 0(判定mode) → Step 1(确认venue/language) → Step 2(证据审计) → Step 3(文献检索) → Step 4(实验复核) → Step 5(Section Blueprint) → Step 6(Draft v1) → Step 7(占位符审计+图表生成) → Step 8(证据合规审查) → Step 9(Prose质量门,内化) → Step 10(扩写检查) → Step 11(综合验证) → Step 12(section loop)`
