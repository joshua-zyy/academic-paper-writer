...
description: "Evidence-driven, section-by-section CS/AI/ML paper drafting orchestrator with Draft→Quality Gate→Verification loop. Coordinates evidence audit, citation, experiments, polishing, review, and figure generation."
---
...
详见 `references/figure-generation-guide.md`。核心要点：

- **Figure Contract 前置**：生成任何图表前必须先定义 core conclusion → evidence chain → archetype → export contract
- **双路径**：架构图→提示词写入 `figures/figure_prompts.md`；数据图→代码写入 `figures/plot_*.py`，不自动执行
- **绘图代码规范**：强制 font/SVG 初始化、学术色板、多面板反冗余
...
本 Skill 与其委托的子 Skill 之间通过规范化数据契约交换信息。数据契约定义在各子 Skill 的 `references/schemas/` 中：

| 契约 | 生产者 → 消费者 | 用途 |
|------|----------------|------|
| `academic-experiments/references/schemas/evidence-inventory.md` | `academic-experiments`, Step 2 → Step 6 | 实验证据盘点数据 |
| `academic-citation/references/schemas/verified-references.md` | `academic-citation` → Step 6 | 核验后的文献引用数据 |
| `academic-citation/references/schemas/literature-reading-report.md` | `literature-reader-agent` → Step 3 | 文献阅读提炼数据 |
| `academic-reviser/references/schemas/verification-report.md` | `academic-reviser` → Step 11 | 审修验证状态数据 |

各 Skill 的独立参考文件：

| 文件 | 用途 |
|------|------|
| `references/evidence-classification.md` | 三类证据的定义与使用规范 |
| `references/placeholder-guide.md` | 占位符系统规范 |
| `references/paper-types.md` | 论文类型定义与选择方法 |
...
| 步骤 | 子 Skill | Agent 文件 | 约束 |
|------|---------|-----------|------|
| Step 2 | `academic-paper-writer` | `agents/probe-agent.md` | 只读，禁止修改项目文件 |
| Step 3 | `academic-citation` | `agents/citation_agent.md` | 只检索，禁止修改项目文件 |
| Step 3a/3b | `academic-citation` | `agents/literature-reader-agent.md` | 只阅读，输出 LiteratureReadingReport，可并行 dispatch |
| Step 4 | `academic-experiments` | `agents/experiment_agent.md` | 可运行实验，禁止修改源代码/数据 |
| Step 7 | `academic-figure` | `agents/figure_agent.md` | 只生成图表，禁止修改项目代码 |
| Step 8/11 | `academic-reviser` | `agents/reviser_agent.md` | 只审查文本，禁止修改项目文件 |

### 内化调用

| 步骤 | Skill | 说明 |
|------|-------|------|
| Step 9 | `academic-polishing` | 主 Agent 读取规则后自行执行，确保风格一致 |

### 职责边界

- **主 Agent**：Section Blueprint、Draft v1、Expansion Pass、Cumulative Draft、Abstract、跨节一致性
- **子 Agent**：提供专项输出（探查/文献/实验/审修/图表），供主 Agent 吸收
- 子 Agent **不得**直接修改 Cumulative Draft 或独立撰写章节
...
| `references/figure-generation-guide.md` | Step 7 生成图表提示词或绘图代码时 |
| `references/exemplar-introduction.md` 等 | 写 Introduction / Related Work / Method / Experiments / Abstract 前 |
| `references/evidence-classification.md` | Step 2 审计证据时 |
| `references/placeholder-guide.md` | Step 6 生成 Draft 使用占位符时 |
| `references/mode-spectrum.md` | 选择或理解任务模式时（Step 0） |
| `references/data-access-levels.md` | 理解跨技能数据访问边界时 |
| `scripts/convert-pdfs-to-md.py` | Step 1b PDF→MD 转换脚本 |
